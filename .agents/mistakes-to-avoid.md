...
| `backend`  | **Never**  | Use XSLT 2.0 functions (e.g., `format-dateTime()`) in stylesheets processed by lxml                                                                                        | lxml only supports XSLT 1.0; XSLT 2.0 functions silently produce empty output; use XSLT 1.0 `<xsl:value-of select="$props//fieldName"/>` patterns instead        |
| `backend`  | **Always** | Wrap best-effort post-success side-effects (e.g., updating GeoNetwork after ingestion) in try/except with `logger.warning()` — never let the exception propagate           | A failure in an optional step must not turn a successful ingestion into an apparent failure; the primary operation's success must be reported accurately         |
| `backend`  | **Always** | When replacing an ISO XML date node to hold a datetime value, change the element tag to `gco:DateTime` (not just the text); never write a datetime string into `gco:Date`  | ISO 19115 and 19139 distinguish `gco:Date` (YYYY-MM-DD) from `gco:DateTime` (full ISO 8601); using the wrong tag causes GeoNetwork schema-validation errors      |
| `backend`  | **Prefer** | Define ISO XML namespace maps used by both service methods and tests as module-level exported constants, not as class attributes                                           | Module-level constants are directly importable in tests (`from src.services.foo import NS_19115_3`); class attributes can't be imported without the class itself |
