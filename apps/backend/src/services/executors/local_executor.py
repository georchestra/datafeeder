...
Runs the staging/process logic directly inside the backend process, synchronously
and iteratively, instead of delegating to Airflow. trigger_staging_task and
trigger_process_task block until the run completes. Only the "direct" flows
triggered by the backend API are supported (initial/rerun staging, process from
an existing staging table) — the scheduled re-ingestion pipeline generated by
...
class LocalTaskExecutor(BaseTaskExecutor):
    ...
    def trigger_staging_task(
        ...
        return TaskRunInfo(
            task_id=_STAGING_DAG_ID,
            run_id=run_id,
            status=self._registry[(_STAGING_DAG_ID, run_id)].status,
            execution_date=None,
        )
    ...
    def trigger_process_task(
        ...
        return TaskRunInfo(
            task_id=_PROCESS_DAG_ID,
            run_id=run_id,
            status=self._registry[(_PROCESS_DAG_ID, run_id)].status,
            execution_date=None,
        )
