...
- [x] T018 [P] [US5] Create `ColumnActionMenuComponent` (presentational, OnPush): dropdown menu with action items (remove, change type, filter), indicator dots per action, emits selected action via `@Output()` in `apps/frontend/src/app/shared/components/column-action-menu/column-action-menu.component.ts`
- [x] T019 [P] [US5] Create `ColumnHeaderComponent` (presentational, OnPush): action button with indicator dot, `@Input()` for column config, `@Output()` for action events in `apps/frontend/src/app/shared/components/column-header/column-header.component.ts`
- [x] T020 [US5] Refactor `DatasetPreviewTableComponent` to use `ColumnHeaderComponent` in column headers, pass column config and handle action events in `apps/frontend/src/app/shared/components/dataset-preview-table/dataset-preview-table.component.ts`
- [x] T021 [US5] Refactor `DataImportWizardComponent` config flow: build column config signal from metadata, chain PUT metadata → GET preview with `switchMap`, implement `raw=true` fallback on preview error (pipe through `timeout(5000)`; on `TimeoutError` or HTTP error call GET preview with `raw=true` and display raw data + error message — do NOT roll back the PUT already persisted). The config signal includes all transformation fields: `columns` AND `force_projection` (from `DatasetConfigurationComponent`). On preview error, show raw data + error message without reverting the config panel. In `apps/frontend/src/app/shared/components/data-import-wizard/data-import-wizard.component.ts`
