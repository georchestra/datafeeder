...
class TestDatafeeder:
    ...
    def validate_and_assert_feature_count(self, page: Page, expected_number_of_features: int | None, timeout: int):
        ...
        if expected_number_of_features:
            ...
            integrity_link = page.request.get(f"/datafeeder-backend/ingestion/integrity-link/{integrity_link_id}")
            ...
            print(f'hits: {number_matched} expected: {expected_number_of_features} for {integrity_link.json()["data_id"]}')
    ...
    def test_import_database(self, page: Page):
        ...
        page.get_by_role("textbox", name="https://").fill("https://www.data.gouv.fr/api/1/datasets/r/47ac11c2-8a00-46a7-9fa8-9b802643f975")
        page.locator("gn-ui-url-input").get_by_role("button").click()
        page.get_by_role("button", name="Configure the dataset").click()
        ...
        page.get_by_role("columnheader", name="Département principal de dé").get_by_role("button").click()
