...
if missing_env_vars:
    pytest.skip(
        f"Missing env vars: {', '.join(missing_env_vars)}; skipping credential-based tests",
...
class TestDatafeederCredentials:
    def test_import_ftp(self, page: Page):
        ...
        page.locator("app-data-source-ftp form div").filter(has_text="Username* Password*").locator(
            'input[type="text"]'
        ).click()
        page.locator("app-data-source-ftp form div").filter(has_text="Username* Password*").locator(
            'input[type="text"]'
        ...
        expect(page.get_by_placeholder("Enter a title for your dataset")).to_have_value(
            expected_title, timeout=15000
        )
        page.get_by_role("button", name="Validate the dataset").click()
        expect(page.locator('[data-test="recordTitleInput"]')).to_have_value(
            expected_title, timeout=15000
        )
