...
### Requirement: Suppression d'un JDD par son propriétaire

Un utilisateur authentifié SHALL pouvoir supprimer un JDD dont il est le propriétaire depuis le tableau de bord de ses données.

#### Scenario: Suppression réussie par le propriétaire

- **WHEN** un utilisateur envoie `DELETE /api/ingestion/integrity-link/{id}` pour un JDD dont il est le propriétaire
- **THEN** le système retourne HTTP 204
- **THEN** le JDD n'est plus accessible via `GET /api/ingestion/integrity-links/`

#### Scenario: Refus de suppression pour un non-propriétaire

- **WHEN** un utilisateur envoie `DELETE /api/ingestion/integrity-link/{id}` pour un JDD dont il n'est pas le propriétaire
- **THEN** le système retourne HTTP 403

#### Scenario: JDD inexistant
...
Un administrateur SHALL pouvoir supprimer n'importe quel JDD, quel que soit son propriétaire.

#### Scenario: Suppression par un admin d'un JDD appartenant à un autre utilisateur

- **WHEN** un administrateur envoie `DELETE /api/ingestion/integrity-link/{id}` pour un JDD appartenant à un autre utilisateur
- **THEN** le système retourne HTTP 204
- **THEN** le JDD n'est plus accessible

### Requirement: Nettoyage des ressources associées lors de la suppression

Lors de la suppression d'un JDD, le système SHALL nettoyer toutes les ressources associées dans l'ordre suivant : DAG Airflow (si récurrence), couche GeoServer, table de données finale, fiche GeoNetwork, enregistrement IntegrityLink (avec cascade sur IntegrityLinkRule).

#### Scenario: Suppression complète d'un JDD avec récurrence

- **WHEN** un JDD avec un `schedule` défini est supprimé
...
#### Scenario: Suppression d'un JDD sans récurrence

- **WHEN** un JDD sans `schedule` est supprimé
...
- **THEN** le JDD reste dans la liste
...
### Requirement: Disparition du JDD de la liste après suppression

Après une suppression réussie, le JDD SHALL disparaître immédiatement de la liste sans rechargement complet de la page.

#### Scenario: Suppression confirmée par l'utilisateur

- **WHEN** l'utilisateur clique sur l'icône de suppression et la requête DELETE retourne HTTP 204
